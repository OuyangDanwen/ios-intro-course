...
class ContactListViewController: UIViewController {
	...
	*/
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if let image = SimulatedNetwork.loadImage(name: contact.imageName) {
                if let imageData = UIImageJPEGRepresentation(image, 1.0) {
                    newContact.imageData = imageData as NSData
                    self.imageBuffer[contact.imageName] = image
	...
	func unfavorite(contact: Contact, index: Int, completion: @escaping () -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            if let imageName = contact.imageName, let firstName = contact.firstName, let lastName = contact.lastName,
                let phoneNumber = contact.phoneNumber {
                let newContact = ServerContact(imageName: imageName, firstName: firstName, lastName: lastName, phoneNumber: phoneNumber)
