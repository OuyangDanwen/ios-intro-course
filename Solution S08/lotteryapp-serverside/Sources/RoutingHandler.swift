...
func removeLottery(request: HTTPRequest, response: HTTPResponse) {
    ...
    //var (message, statusCode) = ("Success", HTTPResponseStatus.ok)
    bson.append(key: "timestamp", string: timestamp)
    let remove = collection.remove(selector: bson, flag: .singleRemove) as MongoResult
    let (message, statusCode) = parseMongoResult(result: remove,
                                                 defaultAction: ("Success", .ok))
